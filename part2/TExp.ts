...
const parseCompoundTExp = (texps: Sexp[]): Result<TExp> => {
    ...
    return (pos === -1) ? parseTupleTExp(texps) :
           ...
           safe2((args: TupleTExp, returnTE: TExp) => checkProcExp(args,returnTE))
                (parseTupleTExp(texps.slice(0, pos)), parseTExp(texps[pos + 1]));
};

const checkProcExp = (args: TupleTExp, returnTE: TExp): Result<TExp> => 
    isNonEmptyTupleTExp(args) ? makeOk(makeProcTExp(args.TEs, returnTE)) : 
    makeOk(makeProcTExp([],returnTE))
...
const parseTupleTExp = (texps: Sexp[]): Result<TupleTExp> => {
    ...
    return isEmptyTuple(texps) ? makeOk(makeEmptyTupleTExp()) : bind(bind(splitEvenOdds(texps),
                                    (argTEs: Sexp[]) => mapResult(parseTExp, argTEs)),t=>checkTuple(t));
}

const checkTuple = (texps: TExp[]): Result<TupleTExp> => 
    allT(isTExp, texps) ? makeOk(makeNonEmptyTupleTExp(texps)) : makeFailure("works")
/*
