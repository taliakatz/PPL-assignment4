...
import { isAppExp, isBoolExp, isDefineExp, isIfExp, isLetrecExp, isLetExp, isNumExp,
         ...
         Parsed, PrimOp, ProcExp, Program, StrExp, isValuesExp, isLetValuesExp, ValuesExp, CExp, LetValuesExp, ValuesBinding } from "./L5-ast";
import { applyTEnv, makeEmptyTEnv, makeExtendTEnv, TEnv } from "./TEnv";
import { isProcTExp, makeBoolTExp, makeNumTExp, makeProcTExp, makeStrTExp, makeVoidTExp,
         parseTE, unparseTExp,
         BoolTExp, NumTExp, StrTExp, TExp, VoidTExp, makeNonEmptyTupleTExp, makeEmptyTupleTExp, isTExp, isNonEmptyTupleTExp, isTupleTExp, isEmptyTupleTExp } from "./TExp";
import { isEmpty, allT, first, rest } from '../shared/list';
import { Result, makeFailure, bind, makeOk, safe3, safe2, zipWithResult, mapResult, isFailure } from '../shared/result';
...
//the old function we submitted:
// export const typeofNum = (): NumTExp => makeNumTExp();
export const typeofNum = (n: NumExp): NumTExp => makeNumTExp(); 


// a boolean literal has type bool-te
//the old function we submitted:
// export const typeofBool = (): BoolTExp => makeBoolTExp();
export const typeofBool = (b: BoolExp): BoolTExp => makeBoolTExp();


// a string literal has type str-te
//the old function we submitted:
// const typeofStr = (): StrTExp => makeStrTExp();
...
export const typeofLetValuesExp = (exp: LetValuesExp, tenv: TEnv): Result<TExp> =>{
    if( !checkBindings(exp.vBindings, tenv) )
        return makeFailure("Incompatible types");
    const vars : string[] =  exp.vBindings.map((b) => b.var).reduce((acc,curr)=> acc = acc.concat(curr),[]).map(v => v.var)
    const varTEs: TExp[] =  exp.vBindings.map((b) => b.var).reduce((acc,curr)=> acc = acc.concat(curr),[]).map(v => v.texp)
    const valsResult: Result<TExp[]> = mapResult(cexp => typeofExp(cexp, tenv) ,exp.vBindings.map((b) => b.val)) 
    const valsTEs: Result<TExp[]> = bind(valsResult, (t:TExp[]) => makeOk(t.reduce((acc : TExp[],curr : TExp)=> isNonEmptyTupleTExp(curr) ? acc = acc.concat(curr.TEs) : isEmptyTupleTExp(curr) ? acc : acc = acc.concat(curr), [])))
    const constraints = bind(valsTEs, (TEvals: TExp[]) => zipWithResult((t, v) => checkEqualType(t,v,exp), TEvals, varTEs))
    return bind(constraints, _ => typeofExps(exp.body, makeExtendTEnv(vars, varTEs, tenv)));
}

const checkBindings = (bindings: ValuesBinding[], tenv: TEnv): boolean => {
    for (let i = 0; i < bindings.length; i++){
        const res: Result<TExp> = typeofExp(bindings[i].val, tenv);
        if(isFailure(res) || !isTupleTExp(res.value))
            return false;
        if(!isTupleTExp(res.value) || isNonEmptyTupleTExp(res.value) && res.value.TEs.length  !== bindings[i].var.length)
            return false;
        if(isEmptyTupleTExp(res.value) && bindings[i].var.length != 0) 
            return false;
...
export const typeofValuesExp = (exp: ValuesExp, tenv: TEnv): Result<TExp> => 
    bind(mapResult((val : CExp)=> typeofExp(val, tenv), exp.vals),
                (texp)=> isEmpty(texp) ? makeOk(makeEmptyTupleTExp()) : allT(isTExp, texp) ? makeOk(makeNonEmptyTupleTExp(texp)) : makeFailure("not a TExp"))
...
// then type<(letrec((p1 (lambda (x11 ... x1n1) body1)) ...) body)>(tenv-body) = א
...
// the old function we submitted:
// export const typeofDefine = (): Result<VoidTExp> => {
//     // return Error("TODO");
...
// the old function we submitted:
// export const typeofProgram = (): Result<TExp> =>
//     makeFailure("TODO");
