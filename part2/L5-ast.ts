...
//         |  ( values <value>*)               / ValuesEx(vals: Value[])
//         |  ( let-valuese <var-decl>* <value>*)               / LetValuesExp(vars:VarDecl[], vals: Value[])
...
export interface LetValuesExp {tag: "LetValuesExp"; vBindings: ValuesBinding[]; body: CExp[]; }
export const makeLetValuesExp = (vBindings: ValuesBinding[], body: CExp[]): LetValuesExp =>
    ({tag: "LetValuesExp",vBindings: vBindings, body: body});
...
export const parseL5SpecialForm = (op: Sexp, params: Sexp[]): Result<CExp> =>
    ...
    op === "let-values" ? parseLetValuesExp(first(params), rest(params)) : //vardecl[], vals+body[]
    makeFailure("Never");
...
export const parseValuesExp = (params: Sexp[]): Result<ValuesExp> =>
    ...
    bind(mapResult(parseL5Exp, params), (exps: Exp[]) =>
        allT(isCExp, exps) ? makeOk(makeValuesExp(exps)) : makeFailure("unexpected type")) 
    
const isGoodValuesBindings = (valuesBindings: Sexp): valuesBindings is [Sexp[], Sexp][] =>
    isArray(valuesBindings) && allT(isArray, valuesBindings) && allT(isArray, valuesBindings.map((b) => b[0]));
...
const parseLetValuesExp = (bindings: Sexp, body: Sexp[]): Result<LetValuesExp> =>
    ...
    safe2((vBinding: ValuesBinding[], body: CExp[]) => makeOk(makeLetValuesExp(vBinding, body)))
...
const parseValuesBindings = (vBindings: [Sexp[], Sexp][]): Result<ValuesBinding[]> =>{
    let varsResult =  map(b => b[0], vBindings).map(b=> mapResult(parseVarDecl, b))
    let emptyArr : VarDecl[][] = []
    if(allT(isOk,varsResult)){
        let varsArray = varsResult.filter((res) => isOk(res)).reduce((acc,curr) => isOk(curr) ? acc=acc.concat([curr.value]) : acc, emptyArr)
        let valsResult = mapResult(parseL5CExp, map(b => b[1], vBindings));
        if(isOk(valsResult)){
            if(isEmpty(varsArray))
                return makeOk([makeValuesBinding([], makeValuesExp([]))])
            return makeOk(zipWith(makeValuesBinding, varsArray, valsResult.value))
        }
    }
...
export const unparse = (e: Parsed): Result<string> =>
    ...
    isValuesExp(e) ?  bind(mapResult(unparse, e.vals), (vals: string[])=>isEmpty(vals) ? makeOk(`(values)`) :  makeOk(`(values ${join(" ", vals)})`)) :
...
const unparseValuesBindings = (vBindings: ValuesBinding[]) : Result<string> =>
    bind(mapResult(bdg => safe2((vd: string, val: string) => makeOk(`(${vd} ${val})`))(unparseVarDeclArray(bdg.var), unparse(bdg.val)), vBindings),
            (bdgs: string[]) => makeOk(join(" ", bdgs)));

const unparseVarDeclArray = (vd : VarDecl[]) : Result<string> =>
    bind(mapResult(unparseVarDecl, vd), (str: string[])=> makeOk(`(${join(" ", str)})`))
...
const unparseLetValuesExp = (le: LetValuesExp) : Result<string> => 
    safe2((bdgs: string, body: string) => makeOk(`(let-values (${bdgs}) ${body})`))
            (unparseValuesBindings(le.vBindings), unparseLExps(le.body));