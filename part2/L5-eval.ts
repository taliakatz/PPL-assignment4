...
import { CExp, Exp, IfExp, LetrecExp, LetExp, ProcExp, Program, SetExp, isCExp, isValuesExp, isLetValuesExp, ValuesExp, LetValuesExp, ValuesBinding } from './L5-ast';
...
import { isClosure, makeClosure, Closure, Value, SExpValue, makeTuple, isTupleSexp } from "./L5-value";
...
const evalValues = (exp: ValuesExp, env: Env): Result<Value> =>
   bind(mapResult(val => applicativeEval(val, env), exp.vals), (sexpVals)=>makeOk(makeTuple(sexpVals)))

// LET: Direct evaluation rule without syntax expansion
// compute the values, extend the env, eval the body.
const evalLetValues = (exp: LetValuesExp, env: Env): Result<Value> => {
    let emptyArr: SExpValue[] = []
    if(checkBindings(exp.vBindings, env)){
        const valuesRes = bind(mapResult((v : CExp) => applicativeEval(v, env), map((b : ValuesBinding) => b.val, exp.vBindings)), (sexp)=>makeOk(sexp));
        const vars = map((b: ValuesBinding) => b.var, exp.vBindings);
        return bind(valuesRes, (vals: SExpValue[]) => evalSequence(exp.body, makeExtEnv((vars.reduce((acc,curr)=> acc = acc.concat(curr),[])).map(v => v.var), vals.reduce((acc, curr) => isTupleSexp(curr) ? acc = acc.concat(curr.val) : acc= acc.concat([curr]),emptyArr), env)));
    }
    return makeFailure("unmatching number of var decls and values")
}


const checkBindings = (bindings: ValuesBinding[], env: Env): boolean => {
    for (let i = 0; i < bindings.length; i++){
        const res = applicativeEval(bindings[i].val, env);
        if(isFailure(res) || !isTupleSexp(res.value))
            return false;
        if(res.value.val.length !== bindings[i].var.length)
            return false; 
